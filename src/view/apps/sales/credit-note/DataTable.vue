<template>
    <DataTableStyleWrap>
        <div v-if="filterOption" class="ninjadash-datatable-filter">
            ...
            <div class="ninjadash-datatable-filter__middle">
                <a-select
            ...
            <div class="ninjadash-datatable-filter__right">
                <h3>
                    Export Data:
                    <span
                        style="cursor: pointer; color: #0d79df; font-size: 14px"
                        @click="exportToPDF"
                        >PDF</span
                    >
                    <span
...
import { DataTableStyleWrap } from "../DataTableStyle";
...
export default defineComponent({
    ...
    setup(_, { emit }) {
        ...
        const exportToPDF = () => {
            ...
            console.log("table rows", selectedRows.value);

            const tableDataExport = selectedRows.value.map((creditNote) => [
                creditNote.creditNoteNumber,
                `${creditNote.clientInfo.user.first_name}` +
                    " " +
                    `${creditNote.clientInfo.user.last_name}`,
                creditNote.dateIssued,
                formatNumberWithCommas(creditNote.creditAmount),
            ...
            const finalFileName = `credit_notes_${new Date().toLocaleDateString()}.pdf`;
        ...
        const exportToCSV = () => {
            ...
            const rows = selectedRows.value.map((creditNote) => [
                creditNote.creditNoteNumber,
                `${creditNote.clientInfo.user.first_name}` +
                    " " +
                    `${creditNote.clientInfo.user.last_name}`,
            ...
            link.setAttribute(
                "download",
                `credit_notes_${new Date().toLocaleDateString()}.csv`,
            );
        ...
        const exportToJSON = () => {
            ...
            const data = selectedRows.value.map((creditNote) => ({
                credit_note_number: creditNote.creditNoteNumberr,
                client:
                    `${creditNote.clientInfo.user.first_name}` +
                    " " +
                    `${creditNote.clientInfo.user.last_name}`,
                issue_date: creditNote.dateIssued,
                credit_amount: creditNote.creditAmount,
            ...
            link.download = `credit_notes_${new Date().toLocaleDateString()}.json`;
            document.body.appendChild(link);
            link.click();
        };
