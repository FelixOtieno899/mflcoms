...
export default defineComponent({
    ...
    setup(_, { emit }) {
        ...
        const exportToPDF = () => {
            ...
            const tableDataExport = selectedRows.value.map((invoice) => [
                invoice.invoice_number,
                `${invoice.clientInfo.user.first_name}` +
                    " " +
                    `${invoice.clientInfo.user.last_name}`,
            ...
            const finalFileName = `invoices_${new Date().toLocaleDateString()}.pdf`;
        ...
        const exportToCSV = () => {
            ...
            const rows = selectedRows.value.map((invoice) => [
                invoice.invoice_number,
                `${invoice.clientInfo.user.first_name}` +
                    " " +
                    `${invoice.clientInfo.user.last_name}`,
        ...
        const exportToJSON = () => {
            ...
            const data = selectedRows.value.map((invoice) => ({
            ...
            link.download = `invoices_${new Date().toLocaleDateString()}.json`;
            document.body.appendChild(link);
            link.click();
        };
